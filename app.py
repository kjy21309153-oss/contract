...
def fetch_law_text(mst):
    """MST(법령일련번호)로 법령 조문 전문을 가져온다. 삭제된 조문은 '삭제됨'으로
    명시해, AI가 그 조문 내용을 지어내지 못하게 한다."""
    try:
        ...
        for jo in root.findall(".//조문단위"):
            ...
            if "삭제" in content and len(content) < 30:
                # "제4조 삭제 <2020.1.1>" 처럼 삭제 표시만 있는 조문
...
def _register_law_with_decree(law_name):
    ...
    ("OO 및 시행령")으로 보이게 등록한다. 다만 검색·인용 시 법률/시행령을
    혼동하지 않도록, 내부적으로는 sub_texts에 각각 이름표를 붙여 따로 저장한다."""
    display_name = f"{law_name} 및 시행령"
    if display_name in st.session_state.knowledge_base:
        return

    sub_texts = {}  # {"실제 법령/시행령 이름": 본문}
    시행일자 = ""
    시행일자_시행령 = ""
    ...
    if LAW_API_OC:
        ...
        if info and info["MST"]:
            ...
            if fetched:
                sub_texts[law_name] = fetched
                시행일자 = info["시행일자"]

        info2 = search_law(f"{law_name} 시행령")
        if info2 and info2["MST"]:
            fetched2 = fetch_law_text(info2["MST"])
            if fetched2:
                sub_texts[f"{law_name} 시행령"] = fetched2
                시행일자_시행령 = info2["시행일자"]

    st.session_state.knowledge_base[display_name] = {
        "text": "\n\n".join(f"[{k}]\n{v}" for k, v in sub_texts.items()),
        "sub_texts": sub_texts,
        "uploaded_at": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
        "시행일자": 시행일자,
        "시행일자_시행령": 시행일자_시행령,
...
def refresh_all_documents():
    ...
    for name, doc in list(st.session_state.knowledge_base.items()):
        if name.endswith(" 및 시행령"):
            ...
            sub_texts = dict(doc.get("sub_texts", {}))
            if law_info and law_info["MST"]:
                law_text = fetch_law_text(law_info["MST"])
                if law_text:
                    sub_texts[base_name] = law_text
                    doc["시행일자"] = law_info["시행일자"]
            if decree_info and decree_info["MST"]:
                decree_text = fetch_law_text(decree_info["MST"])
                if decree_text:
                    sub_texts[f"{base_name} 시행령"] = decree_text
                    doc["시행일자_시행령"] = decree_info["시행일자"]

            if sub_texts:
                doc["sub_texts"] = sub_texts
                doc["text"] = "\n\n".join(f"[{k}]\n{v}" for k, v in sub_texts.items())
...
def search_relevant_chunks(query, top_k=4):
    ...
    for name, doc in st.session_state.knowledge_base.items():
        if doc.get("sub_texts"):
            # 법률/시행령처럼 내부적으로 나뉜 문서는, 각자의 정확한 이름으로 따로 청크·라벨링한다
            # (그래야 AI가 "이건 법률 제O조", "이건 시행령 제O조"를 혼동하지 않는다)
            for sub_name, sub_text in doc["sub_texts"].items():
                for c in chunk_text(sub_text):
                    all_chunks.append(c)

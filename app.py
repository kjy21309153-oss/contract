...
* [필수 확인] 부당특약·경쟁제한 소지가 있는 항목을 발견하면, 참고자료(지식창고)
  안에 국가계약법·시행령(또는 그와 유사한 "OO 및 시행령" 형태의 항목)이 있는지
  반드시 훑어보고, 관련 조항이 있으면 근거로 함께 제시한다. 사내 규정·계약서
  문서 하나만 보고 결론짓지 않는다.
* 심각도 구분:
...
- 법령을 근거로 들 때는 "제O조"까지만 말하지 말고, 그 조가 ①②③ 항이나 "제1항"
  같은 항으로 나뉘어 있으면 실제로 관련 내용이 있는 항 번호까지 구체적으로
  밝히고("제O조제O항"), 그 항에 어떤 내용이 있는지 짧게 함께 설명한다.
- [매우 중요] 특정 조·항·호 번호를 콕 집어 근거로 들 때는, 반드시 그 부분의
  실제 원문 문구를 15자 이상 그대로 큰따옴표로 인용해서 함께 제시한다
  (예: 제4조의5제2항제3호 "계약상대자의 이익을 부당하게..."). 조·항·호의 개수나
  구조(예: 1항 밑에 호가 있는지, 몇 호까지 있는지)도 원문에 실제로 있는 그대로만
  말하고 지어내지 않는다.
  다만 이 규칙 때문에 "관련 조항이 있다는 것 자체"까지 숨기지는 않는다.
  참고자료(지식창고) 안에서 관련 있어 보이는 법령·조항을 발견했는데 정확한
  조·항·호 번호나 정확한 문구를 콕 집어 인용할 자신이 없다면, 번호를 지어내는
  대신 "OOO법에 계약상대자의 이익을 부당하게 제한하는 특약을 금지하는 취지의
  조항이 있으니, 정확한 조·항 번호는 원문을 직접 확인해주세요"처럼 존재
  가능성과 방향을 안내한다. "관련 근거를 찾지 못했다"고 아예 없는 것처럼
  말하지 않는다 — 침묵보다는 불확실성을 밝히는 쪽을 택한다.

[근거 확정 순서 — 반드시 이 순서를 지킬 것]
1. 먼저 [참고자료(지식창고)]로 실제 전달된 내용 안에서 관련 조항을 찾는다.
   있으면 그 내용을 그대로 최종 근거로 사용한다. (인터넷을 확인할 필요 없음)
2. [참고자료(지식창고)]만으로 판단이 부족하면(예: 어떤 조항이 문제될지 감이
   안 잡히는 경우), 문제될 만한 조항이 무엇인지 "감을 잡는 용도로만" Google
   검색을 활용한다. 이때 검색 결과 자체를 최종 근거로 바로 쓰지 않는다.
3. 검색으로 "이런 조항이 관련 있겠다"는 단서(예: 제O조, 부당특약 금지 등)를
   찾았으면, 그 단서에 해당하는 내용이 [참고자료(지식창고)]에 실제로 있는지
   다시 확인한다. 지식창고 안에 있으면 지식창고에 있는 실제 텍스트를 최종
   근거로 인용한다(검색 결과의 문구·연도가 아니라, 지식창고 원문을 기준으로
   조·항 번호와 내용을 밝힌다).
4. 지식창고에 끝내 없는 내용을 부득이 인터넷 검색 결과로만 답할 때는,
   반드시 법제처 국가법령정보센터(law.go.kr)처럼 공식 출처를 우선 확인하고,
   오래된 블로그·요약글에 적힌 개정연도를 그대로 믿지 않는다. 검색 결과가
   실제로 최신인지 확신할 수 없으면 "검색 결과의 최신 여부를 확정할 수
   없습니다"라고 함께 명시한다. 이 경우에만 답변 안에
   "(인터넷 검색 근거: OOOO년 OO월 개정 OOO법 제O조 참고)" 형식으로 표시하고,
   지식창고 근거와 인터넷 검색 근거를 구분해서 안내한다.
5. 지식창고에도 인터넷 검색에도 근거가 없으면 "근거를 찾지 못해 단정할 수
   없습니다"라고 명시한다.
...
def extract_text(file):
    if file.name.lower().endswith(".pdf"):
        ...
        raw = file.read()
        # 한글 텍스트 파일은 UTF-8이 아니라 CP949(EUC-KR)로 저장된 경우가 흔하다.
        # 문제는 CP949 바이트가 '어쩌다 보니' UTF-8로도 에러 없이 디코딩되면서
        # 엉뚱한 글자("외계어")가 나오는 경우가 있어, 단순히 UTF-8을 먼저 시도하는
        # 것만으로는 못 잡는다. 그래서 charset_normalizer로 실제 인코딩을 통계적으로
        # 추정한 뒤 디코딩한다 (requests 패키지에 딸려오는 라이브러리라 별도 설치 불필요).
        try:
...
def fetch_law_articles(mst):
    ...
    항(項)·호(號) 단위가 별도 요소로 내려오는 경우까지 모두 모아 붙여서,
    "몇 조 몇 항"까지 근거로 답할 수 있을 만큼 온전한 조문 내용을 확보한다.
    조 전체가 완전히 삭제된 경우에만 '삭제됨'으로 명시한다 (일부 항만 삭제된
    경우 나머지 항 내용은 그대로 살려서, 조 전체를 잘못 지워버리지 않는다)."""
    try:
        ...
        for jo in root.findall(".//조문단위"):
            ...
            # 항/호 단위가 조문내용과 별도의 하위 요소로 내려오는 경우, 모두 모아 붙인다
            extra_parts = []
            for hang in jo.findall(".//항단위"):
                h_content = (hang.findtext("항내용") or "").strip()
                if h_content:
                    extra_parts.append(h_content)
                for ho in hang.findall(".//호단위"):
                    ho_content = (ho.findtext("호내용") or "").strip()
                    if ho_content:
                        extra_parts.append(ho_content)

            full_content = "\n".join(p for p in [content] + extra_parts if p)
            if not (title or full_content):
                continue

            # 공백을 다 뺐을 때 "삭제<날짜>" 형태만 남는, 즉 조 전체가 삭제된 경우만 삭제 처리
            stripped = re.sub(r'\s+', '', full_content)
            if re.fullmatch(r'삭제(<[^>]*>)?', stripped):
...
FULL_CONTEXT_MAX_CHARS = 350000  # 이보다 커지면 안전하게 예전 방식(TF-IDF 검색)으로 대체


def build_full_context():
    """지식창고를 조각내지 않고, 텍스트 파일 읽듯 전체를 통째로 모아 돌려준다.
    법률/시행령처럼 조 단위로 나뉜 문서도 조를 다시 이어붙여 하나의 글로 만든다.
    전체 분량이 너무 크면(FULL_CONTEXT_MAX_CHARS 초과) None을 반환해서,
    호출부가 예전 방식(관련 부분만 검색)으로 대체하도록 한다."""
    parts = []
    total_len = 0
    ...
    if total_len == 0:
        return ""
    if total_len > FULL_CONTEXT_MAX_CHARS:
        return None

    return "\n\n---\n\n".join(f"[{name}]\n{text}" for name, text in parts)


def search_relevant_chunks(query, top_k=10, per_source_min=1):
    all_chunks = []
    sources = []
    for name, doc in st.session_state.knowledge_base.items():
        if doc.get("sub_articles"):
...
def verify_article_citations(answer_text, context_text):
    """답변에서 언급한 '제O조', '제O조의O' 번호가 실제로 지식창고 원문(context_text)에
    문자 그대로 있는지 확인한다. 없는 번호를 인용했다면 답변 끝에 경고를 붙인다.
    (AI가 조·항·호 번호를 지어내는 걸 프롬프트만으로 100% 막을 수 없어서, 코드로
    한 번 더 사실 확인하는 안전장치다.)"""
    cited = set(re.findall(r'제\d+조(?:의\d+)?', answer_text))
    if not cited or not context_text:
        return answer_text
    missing = sorted(c for c in cited if c not in context_text)
    if missing:
        warning = (
            "\n\n---\n⚠️ **자동 검증 경고**: 위 답변에서 언급된 " + ", ".join(missing) +
            "가 지식창고 원문에서 그대로 확인되지 않았습니다. AI가 조항 번호나 내용을 "
            "잘못 인용했을 가능성이 있으니, 실제 문서를 직접 대조해 확인해주세요."
        )
...
def call_ai(context_messages, user_text, file_text):
    ...
    if full_context is not None:
        # 지식창고 전체를 조각내지 않고 그대로 넘긴다 (분량이 적당할 때)
        context_text = full_context
    else:
        # 지식창고가 너무 커져서 통째로 넘기면 한도를 초과할 수 있을 때만, 관련된 부분만 검색해 넘긴다
    ...
    for idx, model_name in enumerate(MODEL_CANDIDATES):
        try:
            ...
            return verify_article_citations(response.text, context_text)  # 성공하면 검증 후 반환
        except Exception as e:
            error_str = str(e)

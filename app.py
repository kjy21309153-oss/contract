...
- [매우 중요] 특정 조·항·호 번호를 콕 집어 근거로 들 때는, 반드시 그 부분의
  ...
  (예: 제4조의5제2항제3호 "계약상대자의 이익을 부당하게..."). 지금 눈앞에 주어진
  텍스트에서 그 문구를 실제로 찾아 옮길 수 없다면, 그 조·항·호 번호 자체를
  절대 언급하지 않는다 — "이 조가 이런 내용일 것이다"라고 요약하거나 짐작해서
  번호를 붙이지 않는다. 조·항·호의 개수나 구조(예: 1항 밑에 호가 있는지, 몇
  호까지 있는지)도 원문에 실제로 있는 그대로만 말하고 지어내지 않는다.

...
def refresh_all_documents():
    ...
    if not LAW_API_OC:
        st.warning("법제처 API 키(LAW_API_OC)가 설정되지 않아 자동 대조를 건너뛰었습니다. "
    ...
    for name, doc in list(st.session_state.knowledge_base.items()):
        law_name = os.path.splitext(name)[0]  # 확장자 뗀 파일명을 법령명으로 간주
    ...
    if updated:
        st.success("✅ 최신본으로 교체됨: " + ", ".join(updated))
    if unchanged:
        st.caption("변동 없음(이미 최신): " + ", ".join(unchanged))
    if not_found:
        st.caption("법령 검색에서 못 찾음(파일명이 정확한 법령명과 다르거나, 법령이 아닌 자료): "
                   + ", ".join(not_found))

...
if chat_value:
    ...
    if attached_file is not None:
        file_text = extract_text(attached_file)

    if user_text and attached_file is not None:
        display_text = f"📎 {attached_file.name}\n\n{user_text}"
    elif attached_file is not None:
        display_text = f"📎 {attached_file.name}"
    else:
        display_text = user_text

    current["messages"].append({"role": "user", "text": display_text})
    if current["title"] == "새 대화":
        title_source = user_text if user_text else (attached_file.name if attached_file else "새 대화")
    ...
    with st.chat_message("assistant"):
        with st.spinner("검토 중..."):
            try:
                ...
                answer_text = f"⚠️ 답변 생성 중 오류가 발생했습니다: {e}"

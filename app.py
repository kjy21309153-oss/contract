...
MAX_DOCS = 20  # 무료 서버 메모리 한도를 고려한 지식창고 문서 개수 상한
...
[내부 지식 우선 원칙 — 반드시 지킬 것]
질문 유형과 상관없이, [참고자료(지식창고)]로 실제 전달된 내용을 항상 최우선 근거로
사용한다. 그 안에 관련 근거가 하나라도 있으면 반드시 그것부터 사용하고, "혹시
더 정확한 게 있을까" 하는 이유로 인터넷 검색으로 대체하지 않는다.
[지식창고에 있는 문서 제목 목록]은 어떤 문서들이 있는지 참고만 하는 목록일 뿐,
제목만 보고 "관련 없어 보인다"고 판단해 검색으로 넘어가는 근거로 쓰지 않는다.
아래 웹 검색은 [참고자료(지식창고)]로 실제 전달된 내용을 다 확인해봐도
관련 근거가 전혀 없을 때만 사용한다.

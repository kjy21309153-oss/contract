...
for i, msg in enumerate(current["messages"]):
    ...
    if is_user and editing_index == i:
        edited = st.text_area("메시지 수정", value=msg.get("raw_text", msg["text"]),
                               key=f"edit_area_{i}", label_visibility="collapsed")
        c1, c2 = st.columns([1, 1])
        if c1.button("저장 후 재전송", key=f"save_edit_{i}", use_container_width=True):
            file_text = msg.get("file_text", "")
            file_name = msg.get("file_name")
            current["messages"] = current["messages"][:i]
            del st.session_state["editing_index"]
            run_new_turn(current, edited.strip(), file_text, file_name)
            st.rerun()
        if c2.button("취소", key=f"cancel_edit_{i}", use_container_width=True):
            del st.session_state["editing_index"]
            st.rerun()
        continue
    ...
    with st.chat_message("user" if is_user else "assistant"):
        ...
        if is_user:
            _, b1, b2, b3 = st.columns([10, 1, 1, 1])
            if b1.button("🔁", key=f"retry_{i}", help="재시도"):
                current["messages"] = current["messages"][:i + 1]
                answer_text = call_ai(current["messages"][:i], msg.get("raw_text", msg["text"]),
                                       msg.get("file_text", ""))
                current["messages"].append({"role": "model", "text": answer_text})
                save_state()
                st.rerun()
            if b2.button("✏️", key=f"edit_{i}", help="수정"):
                st.session_state["editing_index"] = i
                st.rerun()
            with b3.popover("📋", help="복사"):
                st.code(msg.get("raw_text", msg["text"]), language=None)
        else:
            _, b1 = st.columns([12, 1])
            with b1.popover("📋", help="복사"):
